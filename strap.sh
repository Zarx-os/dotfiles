...
get_mirror()
{
  mirror_p="/bedrock/strata/arch/etc/pacman.d"
...
update_pacman_conf()
{
  ...
  sed -i '/blackarch/{N;d}' /bedrock/strata/arch/etc/pacman.conf

  cat >> "/bedrock/strata/arch/etc/pacman.conf" << EOF
[blackarch]
Include = /bedrock/strata/arch/etc/pacman.d/$MIRROR_F
EOF
}
